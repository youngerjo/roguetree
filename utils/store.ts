...
function getRandomWeather(): Weather {
  return ["sunny", "partly-sunny", "cloudy", "rainy", "stormy", "snowy"][
...
export const useBearStore = create<BearState>()((set) => ({
  ...
  tickStatus: (deltaTime) =>
    set(({ weather, health, sunlight, water, nutrition, stats }) => {
      ...
      const waterAbsorbed =
        stats.waterAbsorptionArea *
        0.01 *
        (stats.waterAbsorptionEfficiency * 0.01);

      const nutritionAbsorbed =
        nutrition *
        (stats.nutritionDigestEfficiency * 0.01) *
        (stats.nutritionReachability * 0.01);

      newHealth += deltaTime * stats.durability * 0.001;
      newHealth +=
        deltaTime * (stats.photosynthesisArea * 0.01) * (waterAbsorbed * 0.01);
  ...
  collectExp: (value) =>
    set(({ level, exp, numPendingUpgrades }) => {
      ...
      const expProgress = clampPercent(
        ((exp - prevExp) / (nextExp - prevExp)) * 100
      );
